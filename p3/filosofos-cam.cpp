...
Cena de los filosofos con camarero
...
void funcion_camarero()
{
  ...
  const int etiq_sentarse = 1;
  const int etiq_levantarse = 2;
  const int lim_sentados = 4;
  int sentados = 0;
  int etiqueta_valida;

  while ( true )
  {
    if (sentados < lim_sentados){
      etiqueta_valida = MPI_ANY_TAG;
    } else {
      etiqueta_valida = etiq_levantarse;
    }
    MPI_Recv ( &id_filosofo, 1, MPI_INT, MPI_ANY_SOURCE, etiqueta_valida, MPI_COMM_WORLD,&estado );
