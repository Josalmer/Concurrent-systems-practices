...
class EstancoSU : public HoareMonitor {
 private:
   ...
   int contador;
   int vecesfumadas[fumadores];
   bool mostradorLibre;

   CondVar fumador[fumadores];
   CondVar mostrador;
   CondVar contadorCond;
...
EstancoSU::EstancoSU(  )
{
   ...
   for (int i = 0; i < fumadores; i++){
     ...
     vecesfumadas[i] = 0;
     ingrediente_disp[i] = 0;
   }
   mostrador = newCondVar();
   contadorCond = newCondVar();
}
// -----------------------------------------------------------------------------
...
void EstancoSU::recuentofumadores( )
{
  if ( contador < 10 ) {
     contadorCond.wait();
  }
  contador = 0;
  int mayor = vecesfumadas[0];
  int numVicioso = 0;
  for (int i = 0; i < fumadores; i++){
    cout << "Fumador " << i+1 << " ha fumado " << vecesfumadas[i] << " veces." << endl;
    if (mayor < vecesfumadas[i]){
      mayor = vecesfumadas[i];
      numVicioso = i;
    }
  }
  cout << endl << "El fumador más vicioso es: " << numVicioso + 1 << endl;
}

// *****************************************************************************
// funciones de hebras

void funcion_hebra_contador( MRef<EstancoSU> Estanco )
{
  while(true){
    Estanco->recuentofumadores();
  }
}
// -----------------------------------------------------------------------------
...
int main()
{
   ...
   // poner en marcha la hebra estanquero
     ...
     // poner en marcha la hebra contador
       hebras[fumadores+1] = thread( funcion_hebra_contador, Estanco ) ;
   // esperar a que terminen todas las hebras
   for( int i = 0 ; i < (fumadores + 2) ; i++ )
     hebras[i].join() ;
