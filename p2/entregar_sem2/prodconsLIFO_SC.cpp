...
Productor-Consumidor con cola LIFO y monitor SC
*/
...
// *****************************************************************************
// clase para monitor buffer, version LIFO, semántica SC, nproductores prod. y nconsumidores cons.

class ProdConsLIFOSC
{
 private:
 ...
 public:                    // constructor y métodos públicos
   ProdConsLIFOSC(  ) ;           // constructor
   int  leer();                // extraer un valor (sentencia L) (consumidor)
   void escribir( int valor ); // insertar un valor (sentencia E) (productor)
} ;
// -----------------------------------------------------------------------------

ProdConsLIFOSC::ProdConsLIFOSC(  )
{
   primera_libre = 0 ;
}
// -----------------------------------------------------------------------------
...
void ProdConsLIFOSC::escribir( int valor )
{
   // ganar la exclusión mutua del monitor con una guarda
   unique_lock<mutex> guarda( cerrojo_monitor );
...
void funcion_hebra_productora( ProdConsLIFOSC * monitor , int indiceHebra )
{
...
void funcion_hebra_consumidora( ProdConsLIFOSC * monitor )
{
...
{
   cout << "-------------------------------------------------------------------------------" << endl
        << "Problema de los productores-consumidores Monitor SC, buffer LIFO con " << nproductores << " productores y " << nconsumidores << " consumidores." << endl
        << "-------------------------------------------------------------------------------" << endl
        << flush ;

   ProdConsLIFOSC monitor ;
